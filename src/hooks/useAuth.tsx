...
function AuthProvider({ children }: AuthProviderData) {
  ...
  async function signIn() {
    ...
    try {
      ...
      if (authResponse.type === "success" && authResponse.params.error !== "access_denied") {
        ...
        const userFormatted = userResponse.data.data[0]
  ...
  return <AuthContext.Provider value={{ user, isLoggingOut, isLoggingIn, signIn, signOut }}>{children}</AuthContext.Provider>
}
